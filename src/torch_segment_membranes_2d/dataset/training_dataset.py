...
class MembraneSegmentationDataset(Dataset):
    """Membrane segmentation dataset.

    https://zenodo.org/record/7104305
    - Images are in subfolders of root_dir called 'images' and 'masks'
    - dataset on disk is resampled to 8 Å/px
    ...
    def image_files(self):
        return sorted(self.image_directory.glob("*/*.mrc"))

    @property
    def mask_files(self):
        return sorted(self.mask_directory.glob("*/*.mrc"))
    ...
    def __getitem__(self, idx):
        ...
        dataset_name = image_file.parts[-2]
        filename = self.image_files[idx].name
        image_file = self.image_directory / dataset_name / filename
        mask_file = self.mask_directory / dataset_name / filename
        self.check_files(image_file, mask_file)

        image = torch.tensor(mrcfile.read(image_file), dtype=torch.float32)
        mask = torch.tensor(mrcfile.read(mask_file), dtype=torch.float32)
